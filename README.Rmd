...
* Run the function `wp_post()` that takes the path as argument, create a draft post in your website, uploads all image media stored in the "figs" folder, edits the references to image media and then publishes the post. 
...
On disk your post is stored as index.Rmd and index.md, but before upload to the WordPress API it is transformed to HTML using [`commonmark`](https://github.com/jeroen/commonmark) and a few regular expressions.
